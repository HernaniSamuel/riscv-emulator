...
- **new**: Creates a **new VM instance** and returns `Self`. A possible `VMError` makes it possible to use **error propagation** with `?`.
...
- **set_x:** If `x.get_mut(index)` returns `None()`, it returns a `VMError` indicating an attempt to access an **invalid register address**. If it returns `Some(&u32)` and the **index is not zero**, the value is **dereferenced** and updated to match the value provided in the method parameter. If the **index** is **zero**, it **does not modify the register value** (since **x0 is always zero**).
